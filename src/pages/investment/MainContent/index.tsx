...
const Investment = () => {
  ...
  const getShareholdersData = async () => {
    ...
    try {
      const shareholderResponse = await getShareholders(shareholderParams);
  ...
  const handleModalDownload = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `روند سرمایه گذاری ${summaryDetail.name} در صندوق ${summaryFundName}.xlsx`
      );
      document.body.appendChild(link);
      link.click();
      link.remove();
    ...
    return (
      <>
        <div className="flex flex-col items-center gap-2 mb-5">
          <p>
            ...
            <span className="font-bold">
              {selectedShareholder.share_holder_histories[0]?.fund}
            </span>
          </p>
          <p>
            <span>سرمایه‌گذار: </span>
            <span className="font-bold">{selectedInvestorName}</span>
          </p>
        </div>
        ...
        <UserTable
          ...
          totalRecords={selectedShareholder.share_holder_histories.length}
        ...
        <div className="p-5 pt-12 relative">
          ...
          <div className="flex flex-col gap-5 items-center py-5 justify-center">
            ...
            <div className="flex gap-5">
              ...
              />
              <Button
                label={loadingDownload ? "در حال پردازش" : "دانلود"}
            ...
            >
              {columns.map((col) => (
                <Column
                  ...
                  sortable={
                    col.field === "total_value" || col.field === "num_funds"
                  }
                  style={{
                    width: col.width,
                    textAlign: col.field === "name" ? "right" : "center",
                  }}
                  body={(node) => {
                    const visibleColumns = getVisibleColumns(node.data.type);
                    if (visibleColumns.includes(col.field)) {
                      if (col.body) {
                        return col.body(node);
                      }
                      return col.field === "name" || col.field === "fund"
                        ? node.data[col.field]
                        : col.field === "total_value" || col.field === "value"
                          ? numberFormatter(
                              Math.trunc(node.data[col.field] / 1000000000)
                            )
                          : numberFormatter(node.data[col.field]);
                    }
                    return null;
                  }}
                />
